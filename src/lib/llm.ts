...
ניתוח עומק לעסקאות וחתימות בכדורסל (חובה כשמסומן basketballTransaction):
- כשהכתבה עוסקת ב*עסקה (טרייד), חתימה, מהלך העברות, סוכן חופשי או שחרור* בכדורסל (NBA או כדורסל ישראלי) - אל תסתפק בדיווח היבש; הוסֵף מקטע *ניתוח כדורסל מקצועי ומעמיק*. סמן אותו בכותרת-משנה "## ניתוח" (או כותרת ספציפית קצרה), בדרך כלל לקראת סוף הכתבה.
- מקטע הניתוח חייב לכלול את שלושת הרכיבים האלה:
  1. *איך המהלך משנה את דינמיקת הקבוצה*: תפקידים חדשים, רוטציה וחמישייה, התאמות בין השחקנים, מה הקבוצה מרוויחה (גודל, הגנה, קליעה לשלוש, יצירה) ומה היא מאבדת או מסכנת.
  2. *מה חשוב לדעת*: ההשלכות המעשיות - התאמה לשיטת המשחק של המאמן, מי מרוויח ומי נפגע מהמהלך, היבטים כספיים/חוזיים כשהם ידועים מהמקורות, ומה צפוי הלאה.
  3. *הערכת חשיבת ההנהלה*: כיצד, ככל הנראה, חשבה הנהלת הקבוצה על מבנה הסגל, השיטה הכדורסלית והדינמיקה כשעשתה את המהלך. נסֵח זאת *במפורש כהערכה וניתוח שלנו* ("ככל הנראה", "נראה שההנהלה מכוונת ל...", "מבחינה כדורסלית המהלך מתיישב עם...") - לעולם לא כעובדה ודאית ולא כציטוט מומצא של ההנהלה.
- דיוק: בנֵה את הניתוח על העובדות שסופקו (כולל playerStats כשקיים) ועל היגיון כדורסלי מקצועי. *הערכה וניתוח מנומקים - מותר ורצוי; המצאת עובדות, נתונים, סכומי חוזה או ציטוטים - אסור בהחלט.* אם פרט אינו ידוע, נתח סביב מה שכן ידוע במקום להמציא.
- זהו החריג היחיד לכלל "קצר וצפוף": בכתבת עסקה/חתימה מותר ואף רצוי להאריך ולהעמיק - אך כל משפט בניתוח חייב לשאת תובנה כדורסלית ממשית, לא מילוי ולא חזרה.

כתבות NBA (חובה כשמסומן nbaExpertTake) - ניתוח ודעה מקצועית:
- כתבות בקטגוריית NBA אינן דיווח יבש. כתוב כמומחה כדורסל ישראלי שמנתח את החדשה לעומק ומביע *דעה מקצועית* משלו - בדיוק כמו פרשן ב-sport5 שמדבר על ה-NBA.
- כל כתבת NBA תכלול: (1) מה קרה - העובדות מהמקורות; (2) מקטע *ניתוח ודעה* ("## ניתוח" או "## הזווית שלנו") שבו אתה מנתח את המשמעות הכדורסלית, נותן הערכה והקשר, ומביע עמדה מקצועית מנומקת - *מושפעת מהזוויות והדעות שעלו במקורות שסופקו* (relatedSources/fullText), לא מנותקת מהן.
- כל סוגי החדשות לגיטימיים לסיקור ולניתוח: עסקאות, ביצועים, ניצחונות *וגם* פציעות, קנסות, מעצרים, הרחקות, תבוסות וסקנדלים. אלה חדשות ספורט רגילות - סקֵר ונתֵח אותן ענייני ומקצועי כרגיל, בלי להימנע מהן.
- הדעה היא *ניתוח והערכה מקצועית*, לא המצאת עובדות: מותר ורצוי להעריך, לנתח ולחוות דעה ("מבחינה כדורסלית זה מהלך חכם כי...", "לדעתי המהלך הזה הופך אותם למסוכנים יותר"); אסור להמציא נתונים, ציטוטים או תוצאות. בסס את הדעה על העובדות שסופקו ועל היגיון כדורסלי.
...
4. סיווג: לכל כתבה הקצֵה category (אחת מ-avdija / nba / israeli_basketball / world_football) וגם subcategory - תת-קטגוריה חופשית וקצרה בעברית שמתארת את הנושא המדויק (למשל "פלייאוף NBA", "חוזה והעברות", "יורוליג", "פרמייר ליג", "נבחרת ישראל"). המצא תת-קטגוריות חדשות לפי הצורך.
...
החזר אך ורק אובייקט JSON תקין (ללא טקסט נוסף, ללא code fences) במבנה הבא:
{
  "articles": [
    {
      "category": "avdija" | "nba" | "israeli_basketball" | "world_football",
...
export interface GenerationContext {
  ...
  /** כתבת עסקה/חתימה בכדורסל - חייבת לכלול מקטע ניתוח עומק */
  basketballTransaction?: boolean;
  /** כתבת NBA - חייבת לכלול ניתוח כדורסל ודעה מקצועית של מומחה */
  nbaExpertTake?: boolean;
}

/** הוראת-טריגר למקטע הניתוח, כשהכתבה היא עסקה/חתימה בכדורסל */
const TRANSACTION_ANALYSIS_TRIGGER =
  "\n\nשים לב: זוהי כתבת *עסקה/חתימה בכדורסל*. חובה לכלול מקטע ניתוח עומק " +
  '("## ניתוח") הכולל: (1) איך המהלך משנה את דינמיקת הקבוצה, (2) מה חשוב לדעת, ' +
  "(3) הערכה מנומקת של חשיבת ההנהלה (מבנה הסגל, השיטה והדינמיקה) - כהערכה שלנו, " +
  "לא כעובדה. נתח לעומק על בסיס העובדות שסופקו; אל תמציא נתונים/חוזים/ציטוטים.";

/** הוראת-טריגר לכתבת NBA: ניתוח כדורסל מעמיק + דעה מקצועית (כל סוגי החדשות). */
const NBA_EXPERT_TRIGGER =
  "\n\nשים לב: זוהי כתבת *NBA*. כתוב כמומחה כדורסל: כלול מקטע *ניתוח ודעה* " +
  '("## ניתוח" / "## הזווית שלנו") עם הערכה מקצועית מנומקת המושפעת מהמקורות. ' +
  "סקֵר כל סוג חדשה ענייני (כולל פציעות, קנסות, מעצרים, הרחקות, תבוסות, סקנדלים) - " +
  "אלה חדשות לגיטימיות. חווה דעה וניתוח, אך אל תמציא עובדות/נתונים/ציטוטים.";

/** מצרף את הוראות-הטריגר הרלוונטיות (עסקה בכדורסל / כתבת NBA) להודעה */
function analysisTriggers(ctx: GenerationContext): string {
  return (
    (ctx.basketballTransaction ? TRANSACTION_ANALYSIS_TRIGGER : "") +
    (ctx.nbaExpertTake ? NBA_EXPERT_TRIGGER : "")
  );
}
...
const VALID_CATS = ["avdija", "nba", "israeli_basketball", "world_football"];
